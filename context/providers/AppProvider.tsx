...
export function AppProvider({
  ...
  const setColorScheme = (colorScheme?: string) => {
    if (colorScheme && Object.keys(COLORS).includes(colorScheme)) {
      setColorSchemeInternal(
        Object.keys(COLORS).includes(colorScheme) ? colorScheme : "orange",
      )
    }
  }
  ...
  // Update color scheme when app changes
  useEffect(() => {
    !colorScheme && setColorScheme(app?.themeColor || undefined)
  }, [app?.themeColor])
  ...
  // useEffect(() => {
  //   const app = apps.find((app) => app.slug === getSlugFromPathname(pathname))
  ...
  const setApp = (app: (appWithStore & { image?: string }) | undefined) => {
    const final = app
      ? {
          ...app,
    ...
    setAppInternal(final)

    app?.themeColor && setColorScheme(app.themeColor)
    app?.backgroundColor && setAppTheme(app.backgroundColor)
  }
  ...
  const setAppStatus = (
    ...
    if (appStatus) {
      ...
      // Only replace if a specific path is provided
      replace(path)
    }
    // Don't replace if appStatus is null and no path provided
    // This prevents overriding navigation when clearing appStatus
  }
  ...
  useEffect(() => {
    ...
    if (app?.images?.[2]?.url) {
      ...
      newAppleTouchIcon.href = "/favicon.ico"
    }
