...
import { useChat, useNavigationContext, useApp } from "./context/providers"
...
export default function Agent({
  ...
  // // // console.log("🚀 ~ file: Agent.tsx:283 ~ app:", app)
  // // // console.log("🚀 ~ file: Agent.tsx:283 ~ app.images:", app?.images)
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <>
                          <label>
                            <Checkbox
                              checked={field.value?.includes("Atlas")}
                              onChange={(e) => {
                                const newValue = e.target.checked
                                  ? [...(field.value || []), "Atlas"]
                                  : field.value?.filter((v) => v !== "Atlas")
                          ...
                          <label>
                            <Checkbox
                              checked={field.value?.includes("Peach")}
                              onChange={(e) => {
                                const newValue = e.target.checked
                                  ? [...(field.value || []), "Peach"]
                                  : field.value?.filter((v) => v !== "Peach")
                                field.onChange(newValue)
                              }}
                            >
                              🍑 Peach
                          ...
                          <label>
                            <Checkbox
                              checked={field.value?.includes("Vault")}
                              onChange={(e) => {
                                const newValue = e.target.checked
                                  ? [...(field.value || []), "Vault"]
                                  : field.value?.filter((v) => v !== "Vault")
                                field.onChange(newValue)
                              }}
                            >
                              💰 Vault
                          ...
                          <label>
                            <Checkbox
                              checked={field.value?.includes("Bloom")}
                              onChange={(e) => {
                                const newValue = e.target.checked
                                  ? [...(field.value || []), "Bloom"]
