...
Return ONLY ONE WORD: ${apps.map((a) => a.name).join(", ")}, or "none"`
  ...
  const fetchQuotaInfo = async () => {
    const response = await fetch(`${API_URL}/messages?quota=true`, {
  ...
  useEffect(() => {
    if (suggestSaveApp) {
      // console.log(`🚀 ~ file: Chat.tsx:1069 ~ suggestSaveApp:`, suggestSaveApp)
  ...
  const handleSubmit = async () => {
    ...
    if (requiresSignin && !user) {
      threadId
        ? push(`/threads/${threadId}?signIn=login`)
        : push("/?signIn=login")
      return
    }
    ...
    try {
      ...
      if (artifacts && artifacts.length > 0) {
        ...
        postRequestBody = JSON.stringify({
          ...
          threadId: validate(threadId) ? threadId : null,
      ...
      const userResponse = await fetch(`${API_URL}/messages`, {
        method: "POST",
      ...
      const agentResponse = await fetch(`${API_URL}/ai`, {
        method: "POST",
  ...
  useEffect(() => {
    ...
    const processAction = async () => {
      ...
      try {
        ...
        if (result?.contextMenuAction?.text) {
          ...
          await BrowserInstance?.storage.local.remove(["contextMenuAction"])
    ...
    BrowserInstance?.storage?.onChanged?.addListener(handleStorageChange)

    return () => {
      clearInterval(interval)
      BrowserInstance?.storage?.onChanged?.removeListener(handleStorageChange)
    }
  }, [setInput])
                ...
                {guest && !user && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      router.push(
                        threadId
                          ? `/threads/${threadId}?signIn=register`
                          : "/?signIn=register",
                      )
                    }}
                  >
                    ...
                    onClick={() => {
                      {
                        ...
                        router.push(
                          threadId
                            ? `/threads/${threadId}?subscribe=true`
                            : "/?subscribe=true",
                        )
                      }
                    }}
                    ...
                    >
                      ...
                      {debateAgent && !isChrry ? (
                        ...
                        <button
                          ...
                          style={{
                            color: isChrry ? "var(--shade-6)" : undefined,
                          }}
                          onClick={() => {
                            if (appStatus?.part) {
                              toast.error(t("Agent locked during app creation"))
                ...
                {user && !user?.subscription && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      threadId
                        ? push(`/threads/${threadId}?subscribe=true`)
                        : push("/?subscribe=true")
                    }}
                ...
                {guest && (
                  <button
                    onClick={() => {
                      ...
                      if (guest) {
                        threadId
                          ? router.push(`/threads/${threadId}?signIn=login`)
                          : router.push("/?signIn=login")
                        return
                      }
                      threadId
                        ? router.push(`/threads/${threadId}?subscribe=true`)
                        : router.push("/?subscribe=true")
                    }}
